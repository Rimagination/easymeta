...
**登记快照日期：2026-08-05。** 本登记用于确定“什么要求来自哪里、何时复查、可以怎样蒸馏”，不是官方文件的替代品。开始新协议、重大方法变更和投稿前都应回到官方原文确认当前版本。
...
| `COCHRANE-MISSING-13` | [Cochrane Handbook Chapter 13: Assessing risk of bias due to missing evidence](https://www.cochrane.org/authors/handbooks-and-manuals/handbook/current/chapter-13) | 漏斗图适用性、小研究效应检验、替代解释和 missing-evidence 结论边界 | 每次小研究效应/缺失证据分析冻结和投稿前检查 `current` 页面 | Cochrane 版权；蒸馏“少于 10 通常不检验”和“不对称并非偏倚诊断”等原则，不复制图表或整章 |
| `COCHRANE-UPDATE-6.5` | [Cochrane Handbook Chapter IV: Updating a review](https://www.cochrane.org/authors/handbooks-and-manuals/handbook/current/chapter-iv)；last updated August 2023，载于 v6.5 (2024) | 判断是否更新；重审问题/资格/方法；更新检索、旧新研究数据与偏倚评价；透明报告变化 | 每次更新立项和提交前检查 `current` 章节及 MECIR 更新要求 | Cochrane 版权；蒸馏为“不能只追加效应行”的更新门，不复制整章或决策图 |
| `COCHRANE-OVERVIEWS-6.5` | [Cochrane Handbook Chapter V: Overviews of Reviews](https://www.cochrane.org/authors/handbooks-and-manuals/handbook/current/chapter-v)；last updated August 2023，载于 v6.5 (2024) | 选择 overview 产品、系统综述作为分析单位、一级研究重叠、定量结果与更新 | 每个 overview/second-order 项目启动、分析锁定和投稿前检查 `current` 章节 | Cochrane 版权；蒸馏重叠决策和统计边界，不复制图表或整章；生态二阶综合另与 CEE/领域方法交叉核对 |
...
| `EFFECT-CONVERSION-2026` | [Lajeunesse, *Converting and Constructing Effect Sizes With the Response Ratio*](https://doi.org/10.1111/ele.70335)，Ecology Letters 29(3):e70335，Version of Record 20 March 2026 | 生态 response ratio 与 Hedges' d 的新转换路线及可构造效应量框架 | 静态论文；每次采用前读取完整方法、公式、边界/离群规则、方差传播与勘误，不只依赖摘要 | 只登记书目与保守决策门；不把新公式升级为所有 lnRR 的通用转换，也不复制受版权保护的公式推导/图表 |
| `QUALITY-SCORE-1999` | [Jüni et al., *The Hazards of Scoring the Quality of Clinical Trials for Meta-analysis*](https://doi.org/10.1001/jama.282.11.1054) | 解释任意质量量表总分和质量加权的结构性风险 | 静态方法研究；与当前设计特异 RoB、review-level appraisal 和 certainty guidance 联用 | 保存书目信息与方法结论；不复制量表表格。用于禁止任意总分权重，不等于禁止所有基于偏倚的预设分层/敏感性 |
...
| `BENCH-NATURE-BIODIV-2025` | [Keck et al., global human impact on biodiversity](https://www.nature.com/articles/s41586-025-08752-2)；[Zenodo v1.0](https://doi.org/10.5281/zenodo.14608770) | local diversity、homogeneity、composition shift 三个 estimand；空间距离对依赖；v1.0 archive/data/code/session 哈希与三个全局作者模型的 22 项计数/数值/收敛 oracle 已冻结 | `verified targeted_reproduction` 只覆盖 `PBL_stats.R` 三个无抽样方差加权的 Gaussian mixed models；无 lockfile，论文与仓库 R 版本不一致，不能复制为默认或声称 full exact reproduction |
...
| `BENCH-NCOMMS-CHENG-2024` | [Cheng et al., shared-control biodiversity effects](https://www.nature.com/articles/s41467-024-48876-z)；[Figshare v1 data/code](https://doi.org/10.6084/m9.figshare.24953433.v1) | 共享对照 `V`、NBE/ΔNBE、独立实验簇和多层推断；v1 三文件的 MD5/SHA-256 与主 all-invader NBE、warming/drought ΔNBE oracle 已冻结 | `tests/source_reproduction_cases.json` 仅把 `code.R` 16–99 行对应的定向模型标为 verified；不等于约 3500 行脚本和全部图表已复现 |
| `BENCH-NATURE-GONCALVES-2025` | [Gonçalves-Souza et al., fragmented-landscape biodiversity](https://www.nature.com/articles/s41586-025-08688-7)；[Zenodo release 0.1.0](https://doi.org/10.5281/zenodo.14885581) | 原始群落矩阵、α/β/γ、Hill 数、grain/extent 与空间配对；release ZIP 与 processed-data 模型 oracle 已冻结 | verified 范围从 `processed_data/diversity_of_2.csv` 开始；原始 taxonomy 的 4009 唯一字符串与论文 4006 taxa 尚未对齐，故 raw-to-paper 全流程仍阻断 |
| `BENCH-ECOLET-ATKINSON-2022` | [Atkinson et al., restoration mean and variability synthesis](https://pmc.ncbi.nlm.nih.gov/articles/PMC9320827/)；[OSF data/code](https://doi.org/10.17605/OSF.IO/4AUCP) | `lnRR`、偏倚校正 `lnCVR`、参考状态和依赖；OSF file revision 1 与冻结 HTML oracle 已登记 | `blocked`：OSF v1 有 9 行疑似互换 reference SE/样本量，公开代码保留 730 个效应而论文/HTML 报 739；OSF 未声明 node license，不能计入 verified reproduction |
| `BENCH-ECOLET-HONG-2022` | [Hong et al., biodiversity × environmental-change interactions](https://pmc.ncbi.nlm.nih.gov/articles/PMC9300022/)；[Figshare v1 result/plot data](https://doi.org/10.6084/m9.figshare.16947451.v1) | 四单元析因交互、ΔNBE、尺度和协方差；Figshare 只够核对图 2–5 的已汇总值 | `HOLD`：缺 1997 个 NBE、469 个 BE slope、抽样方差和代码；`e^0.360-1=43.3%` 与正文“增加 61%”冲突，不能把结果表重绘称作模型复现 |
| `BENCH-NCOMMS-LEFCHECK-2015` | [Lefcheck et al., biodiversity and ecosystem multifunctionality](https://www.nature.com/articles/ncomms7936) | 函数方向、标准化、阈值曲线、函数集依赖和多功能性 estimand | 不选择最显著阈值，不把函数数量当样本量 |
| `BENCH-NATURE-ISBELL-2015` | [Isbell et al., biodiversity and resistance to climate extremes](https://www.nature.com/articles/nature15374) | 重复年份、抗性/恢复力、事件窗口和时间相关 | 不把年份当独立效应，不因包含 AR(1) 就迁移整套模型 |
| `BENCH-NATURE-HOOPER-2012` | [Hooper et al., biodiversity loss and global-change drivers](https://www.nature.com/articles/nature11118)；[NCEAS context](https://www.nceas.ucsb.edu/workinggroups/biodiversity-and-functioning-ecosystems-translating-results-model-experiments) | second-order/cross-meta、一级研究重叠和 estimand 对齐 | 不把 pooled review estimates 当独立观测或按质量总分加权 |
